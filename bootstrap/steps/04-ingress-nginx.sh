...
if helm status rke2-ingress-nginx -n kube-system >/dev/null 2>&1 || \
   kubectl -n kube-system get ds rke2-ingress-nginx-controller >/dev/null 2>&1; then
  log "RKE2 đã có ingress-nginx (kube-system) — bỏ qua cài Helm riêng."
  kubectl -n kube-system rollout status ds/rke2-ingress-nginx-controller --timeout=300s
