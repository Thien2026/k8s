...
func NewRouter(db *pgxpool.Pool, cfg config.Config, rancherClient *rancher.Client) http.Handler {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Post("/projects/{slug}/resources/apply", h.ApplyProjectResources)
		...
		r.Get("/projects/{slug}/backups", h.ListProjectBackups)
		r.Post("/projects/{slug}/backups/restore", h.CreateProjectBackupRestore)
		r.Get("/projects/{slug}/addons", h.ListProjectAddons)
		r.Post("/projects/{slug}/addons/redis/restart", h.RestartRedisAddon)
		r.Post("/projects/{slug}/addons/redis/promote-prod", h.PromoteRedisAddon)
		...
		r.Get("/projects/{slug}/addons/minio/buckets", h.ListMinioBuckets)
		r.Post("/projects/{slug}/addons/minio/buckets", h.CreateMinioBucket)
		r.Patch("/projects/{slug}/addons/minio/buckets/{bucket}", h.UpdateMinioBucketQuota)
		r.Post("/projects/{slug}/addons/minio/buckets/{bucket}/scan/enable", h.EnableMinioBucketScan)
		r.Get("/projects/{slug}/addons/minio/buckets/{bucket}/scans", h.GetMinioBucketScanStatus)
		...
		r.Get("/github/repos/{owner}/{repo}/contents", h.GitHubListRepoContents)
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/admin/backups/targets", h.ListBackupTargets)
			r.Post("/admin/backups/targets", h.UpsertBackupTarget)
			r.Post("/admin/backups/targets/{id}/test", h.TestBackupTarget)
			r.Get("/admin/backups/runs", h.ListBackupRuns)
			r.Post("/admin/backups/runs", h.PostBackupRun)
			r.Get("/admin/projects/{slug}/quota", h.GetProjectQuota)
			r.Patch("/admin/projects/{slug}/quota", h.PatchProjectQuota)
			r.Post("/admin/projects/quota/backfill", h.PostBackfillProjectQuotas)
		})
	})
